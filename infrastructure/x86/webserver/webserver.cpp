...
unsigned webserver::Request(void* ptr_s) {
 ...
 // std::cout << "webserver::webserver Request socket" << ptr_s <<" " << *s.refCounter_ << std::endl;
  std::string line = s.ReceiveLine();
  if (line.empty()) {
//	  std::cout << "webserver::webserver line empty" << ptr_s <<" " << *s.refCounter_ << std::endl;
  ...
  if(req.status_=="toclose"){
	  s.Close();
	//  std::cout << "webserver::webserver Request socket finished early and closed" << req.path_ <<std::endl;
	  return 0;
  } else {
	//  std::cout << "webserver::webserver Request socket didn't finished early and closed" << req.path_ <<std::endl;
  }
...
//  s.SendLine(std::string("Server: ") +serverName);
  ...
  s.SendLine(std::string("Content-Type: ")+req.contentType);
  s.SendLine("Content-Length: " + str_str.str());
  if(!req.xtraheader.empty()) s.SendLine(req.xtraheader);
  s.SendLine("");
  s.SendLine(req.answer_);

  s.Close();
 // std::cout << "webserver::webserver Request socket finished" << ptr_s <<std::endl;
...
webserver::webserver(unsigned int port_to_listen, request_func r) {
  ...
  //  unsigned ret;
//    std::cout << "webserver::webserver socket" << ptr_s <<std::endl;
