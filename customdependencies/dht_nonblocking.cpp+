...
bool DHT_nonblocking::read_nonblocking( )
{
  ...
  // Serial.print(String()+ "\nDHT22 status DHT_BEGIN_MEASUREMENT"+_pin  );
    digitalWrite( _pin, HIGH );
  ...
  // Serial.print(String()+ "\nDHT22 status DHT_BEGIN_MEASUREMENT_2 "+_pin  );
    /* Wait for 250 ms. */
  ...
  case DHT_DO_READING:
  //Serial.print(String()+ "\nDHT22 status DHT_DO_READING "+_pin  );
    // Wait for 20 ms. 
    if( millis( ) - dht_timestamp > 20 )
    {
      ...
      status = read_data( );
       if( status != true )
       {
        Serial.println(String(F("DHT22Reading failed on pin")) +_pin);
		readfailure=true;
       }
    }
    break;
  ...
  case DHT_COOLDOWN:
 // Serial.print(String()+ "\nDHT22 status DHT_COOLDOWN "+_pin  );
...
bool DHT_nonblocking::read_data( )
{
 ...
 if(_type== DHT_TYPE_AM2301) delayMicroseconds( 100 ); // can be only 100us on AM3201		
	else	delayMicroseconds( 700 );	// probably more 
